...
router = APIRouter(prefix="/users/me", tags=["Profil"])
...
async def update_my_profile(body: ProfileUpdateRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
