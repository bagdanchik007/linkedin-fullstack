...
router = APIRouter(prefix="/connections", tags=["Verbindungen"])


@router.post("/request/{user_id}", response_model=ConnectionResponse, status_code=status.HTTP_201_CREATED)
async def send_request(user_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Man kann sich nicht selbst hinzufügen
    if user_id == current_user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Du kannst dir selbst keine Anfrage senden")

    existing = await service.get_connection(db, current_user.id, user_id)
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Verbindungsanfrage bereits gesendet")

    reverse = await service.get_connection(db, user_id, current_user.id)
    if reverse:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Verbindung existiert bereits")

    connection = await service.send_request(db, current_user.id, user_id)

    # Celery Task: Empfänger im Hintergrund benachrichtigen
    notify_connection_request.delay(str(user_id), str(current_user.id), current_user.email)

    return connection


@router.patch("/{connection_id}/accept", response_model=ConnectionResponse)
async def accept_request(connection_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if connection.status != "pending":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Anfrage ist nicht mehr ausstehend")
...
async def reject_request(connection_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    connection = await service.get_connection_by_id(db, connection_id)
    if not connection:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Anfrage nicht gefunden")
...
async def remove_connection(connection_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    connection = await service.get_connection_by_id(db, connection_id)
    if not connection:
...
async def my_connections(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await service.get_my_connections(db, current_user.id)


@router.get("/pending", response_model=list[ConnectionResponse])
async def pending_requests(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def suggestions(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
