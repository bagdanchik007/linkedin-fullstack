...
from app.applications.schemas import ApplicationCreateRequest, ApplicationResponse, ApplicationStatusUpdate
...
router = APIRouter(tags=["Bewerbungen"])


@router.post("/jobs/{job_id}/apply", response_model=ApplicationResponse, status_code=status.HTTP_201_CREATED)
async def apply_for_job(job_id: uuid.UUID, body: ApplicationCreateRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Stelle prüfen ob sie existiert
    job = await get_job_by_id(db, job_id)
    if not job or not job.is_active:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Stelle nicht gefunden oder nicht aktiv")

    # Doppelte Bewerbung verhindern
    existing = await service.get_application_by_user_and_job(db, current_user.id, job_id)
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Du hast dich bereits auf diese Stelle beworben")
...
async def job_applications(job_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def update_status(application_id: uuid.UUID, body: ApplicationStatusUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    notify_application_update.delay(str(application.user_id), job.title if job else "Unbekannte Stelle", body.status)
